...
<form id="registro_credenciales" class="formulario <?= $mostrando_form1 ? 'active' : '' ?>" onsubmit="event.preventDefault(); validarCredenciales();">
    ...
    <label class="texto_input" for="nombre">Introduzca su nombre</label> 
    <input id="nombre" type="text" placeholder="nombre" value="<?= htmlspecialchars($nombre) ?>" autofocus>    <!-- ?= htmlspecialchars($nombre) ? es una variable que viene de php y sirve para que se mantengan los datos una vez se envian por primera vez --> 

    <label class="texto_input" for="institucion">Introduzca su institución</label> 
...
<form id="registro_contraseña" class="formulario <?= !$mostrando_form1 ? 'active' : '' ?>" method="POST" onsubmit="return validarContraseñas();">
    <!-- Div donde se ponen los errores-->
    <div id="erroresPass" class="errores"></div>
    ...
    <label class="texto_input" for="contraseña2">Confirme su contraseña</label> 
